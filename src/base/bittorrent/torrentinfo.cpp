...
#define QBT_ORIG_FILES layout
#else
#define QBT_ORIG_FILES orig_files
#endif
    ...
    : m_nativeInfo {std::make_shared<const lt::torrent_info>(nativeInfo)}
{
    ...
    const lt::file_storage &fileStorage = m_nativeInfo->QBT_ORIG_FILES();
...
QString TorrentInfo::name() const
{
    ...
    return QString::fromStdString(m_nativeInfo->QBT_ORIG_FILES().name());
}
...
Path TorrentInfo::filePath(const int index) const
{
    ...
    return Path(m_nativeInfo->QBT_ORIG_FILES().file_path(m_nativeIndexes[index]));
}
...
qlonglong TorrentInfo::fileSize(const int index) const
{
    ...
    return m_nativeInfo->QBT_ORIG_FILES().file_size(m_nativeIndexes[index]);
}
...
qlonglong TorrentInfo::fileOffset(const int index) const
{
    ...
    return m_nativeInfo->QBT_ORIG_FILES().file_offset(m_nativeIndexes[index]);
}
...
TorrentInfo::PieceRange TorrentInfo::filePieces(const int fileIndex) const
{
    ...
    const lt::file_storage &files = m_nativeInfo->QBT_ORIG_FILES();
