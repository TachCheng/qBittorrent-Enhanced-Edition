...
private slots:
    void testParseQuery2ResponseBaseOffset()
    {
#ifdef Q_OS_WIN
        ...
        // In standard Everything 1.4+, data_offset is relative to basePtr (start of EVERYTHING_IPC_LIST2W)
        list->items[0].flags = 0;
        list->items[0].data_offset = sizeof(EVERYTHING_IPC_LIST2W);
    ...
    void testParseQuery2ResponseItemOffset()
    {
#ifdef Q_OS_WIN
        const wchar_t *sampleName = L"test_file_item.iso";
        const wchar_t *samplePath = L"D:\\Movies";
        const qulonglong sampleSize = 2147483648ULL; // 2 GB
        ...
        // Alternate: data_offset relative to item2
        list->items[0].flags = 0;
        ...
        QCOMPARE(totalMatches, 1);
        QCOMPARE(results.size(), 1);
        QCOMPARE(results[0].name, QStringLiteral("test_file_item.iso"));
        QCOMPARE(results[0].path, QStringLiteral("D:\\Movies"));
        QCOMPARE(results[0].size, sampleSize);
#endif
    }
};
